...
RUN git clone https://github.com/amantur/safari-tensorflow-05MAY2017.git

RUN pip3 install --user --upgrade pip

RUN ["/bin/bash","-c","echo `pwd` && pip3 install --user --upgrade tensorflow"]

RUN ["/bin/bash","-c","echo `pwd` && pip3 install --user --upgrade -r ~/safari-tensorflow-05MAY2017/requirements.txt"]
